...
glm::vec4 Image::getColour(const GLuint _x,const GLuint _y ) const noexcept
{
// make sure were in the image range
    ...
    if(m_channels == 3)
    {
      return glm::vec4(m_data[offset],m_data[offset+1],m_data[offset+2],0.0f);
		}
		else
		{
    return glm::vec4(m_data[offset],m_data[offset+1],m_data[offset+2],m_data[offset+3]);
		}
	}
	else
	{
...
glm::vec4 Image::getColour(const Real _uvX, const Real _uvY ) const noexcept
{
  ...
  if(m_data!=0)
  {
    ...
    {
      return glm::vec4(m_data[offset],m_data[offset+1],m_data[offset+2],1.0);
    }
  }
  else
  {
