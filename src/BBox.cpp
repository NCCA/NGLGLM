...
void BBox::setVAO()
{
    ...
    // std::vector
    m_vao->bind();
    m_vao->setData(SimpleIndexVAO::VertexData(8*sizeof(glm::vec3),m_vert[0].x,sizeof(indices),&indices[0],GL_UNSIGNED_BYTE,GL_STATIC_DRAW));
  ...
  else // we just need to draw the lines
  {
    ...
    m_vao->bind();
    m_vao->setData( SimpleIndexVAO::VertexData(8*sizeof(glm::vec3),m_vert[0].x,sizeof(lindices),&lindices[0],GL_UNSIGNED_BYTE,GL_STATIC_DRAW));
    m_vao->setVertexAttributePointer(0,3,GL_FLOAT,sizeof(glm::vec3),0);
