...
//----------------------------------------------------------------------------------------------------------------------
Obj::Obj(const std::string& _fname  , nglglm::Obj::CalcBB _clacBB)  noexcept :AbstractMesh()
{
    m_vbo=false;
    m_ext=0;
    // set default values
...
void Obj::save(const std::string& _fname)const noexcept
{
  ...
  if (!fileOut.is_open())
  {
    std::cout <<"File : "<<_fname<<" Not founds "<<'\n';
    return;
  }
