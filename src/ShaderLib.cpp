...
nglglm::Shader* ShaderLib::getShader(const std::string &_shaderName) noexcept
{
  nglglm::Shader* shaderPointer;
  auto shader=m_shaders.find(_shaderName);
  // make sure we have a valid shader and program
  if(shader!=m_shaders.end() )
  {
...
{
  (*this)[m_currentShader]->setUniformMatrix4fv(_paramName.c_str(),1,GL_FALSE,&_v0[0][0]);
}
